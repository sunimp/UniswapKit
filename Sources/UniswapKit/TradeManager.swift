...
class TradeManager {
    ...
    private func buildSwapData(receiveAddress: Address, tradeData: TradeData) throws -> SwapData {
        ...
        case .exactIn:
            ...
            method = try buildMethodForExactIn(
                tokenIn: tokenIn,
                tokenOut: tokenOut,
                path: path,
                to: to,
        ...
        case .exactOut:
            ...
            method = try buildMethodForExactOut(
                tokenIn: tokenIn,
                tokenOut: tokenOut,
                path: path,
                to: to,
    ...
    private func buildMethodForExactOut(
        tokenIn: Token,
        tokenOut: Token,
        path: [Address],
        to: Address,
    ...
    private func buildMethodForExactIn(
        tokenIn: Token,
        tokenOut: Token,
        path: [Address],
        to: Address,
        ...
        switch tokenIn {
        case .eth: return SwapExactETHForTokensMethod(
                amountOut: amountOutMin,
                path: path,
                to: to,
                deadline: deadline,
                supportingFeeOnTransfer: supportingFeeOnTransfer
            )
...
//        print("PAIR ADDRESS: \(pairAddress.toHexString())")
        ...
        let data = try await EvmKit.Kit.call(
            ...
            data: GetReservesMethod().encodedABI()
        )
